...
`;
    ...
    async (ctx: any) => {
        ...
        try {
            ...
            const baseQuote = await switchService.getOfframpQuote(
                amount,
            ...
            const bonusQuote = await switchService.getOfframpQuote(
                amount,
                ctx.wizard.state.data.country,
                ctx.wizard.state.data.asset.id,
                ctx.wizard.state.data.currency,
...
💰 <b>You Sell:</b> ${formatAmount(bonusQuote.source.amount)} ${ctx.wizard.state.data.symbol}
💵 <b>You Get:</b> ${formatAmount(bonusQuote.destination.amount)} ${bonusQuote.destination.currency}
`;
...
`;
            ...
            msg += `
📈 <b>Rate:</b> 1 ${ctx.wizard.state.data.symbol} = ${formatAmount(bonusQuote.rate)} ${bonusQuote.destination.currency}
...
`;
    ...
    async (ctx: any) => {
        ...
        if (b.holderName) {
            ...
            const baseAmount = ctx.wizard.state.baseQuote?.destination?.amount || ctx.wizard.state.quote.destination.amount;
...
💵 <b>Receiving:</b> ${formatAmount(bonusAmount)} ${ctx.wizard.state.data.currency}
`;
